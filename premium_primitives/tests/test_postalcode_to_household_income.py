...
from premium_primitives.postalcode_to_household_income import (
...
class TestPostalCodeToHouseholdIncome(BaseTestTransform):
    primitive = PostalCodeToHouseholdIncome

    def test_valid_5digit(self):
        primitive_func = self.primitive().get_function()
        postalcodes = pd.Series(["10001", "82838", "02116"])
        correct_answers = [85221, 59000, 103422]
    ...
    def test_valid_5digit_with_whitespace(self):
        primitive_func = self.primitive().get_function()
        postalcodes = pd.Series([" 10001", "82838 ", " 02116 "])
        correct_answers = [85221, 59000, 103422]
        answers = primitive_func(postalcodes)
    ...
    def test_with_nan(self):
        ...
        correct_answers = [85221, np.nan, 103422]
        answers = primitive_func(postalcodes)
        np.testing.assert_array_equal(answers, correct_answers)
    ...
    def test_bool_input(self):
        ...
        correct_answers = [np.nan, np.nan, 103422]
        answers = primitive_func(postalcodes)
        np.testing.assert_array_equal(answers, correct_answers)
